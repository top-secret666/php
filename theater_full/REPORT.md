...
**Статистика по сеансам.** Для формирования отчетов по посещаемости и продажам используется агрегирующая таблица `performance_stats`:
- `tickets_sold` — количество проданных билетов;
- `checked_in_count` — количество отметок прохода;
- `revenue` — выручка.
...
### Корзина и оформление (checkout)
Для улучшения пользовательского сценария реализована «корзина»:
1. Пользователь бронирует билет(ы) — создаются записи `tickets` со статусом `reserved` без `order_id`.
2. Страница корзины показывает **только** брони текущего пользователя.
3. При оформлении создается `orders`, билеты привязываются к заказу и переводятся в `sold`, устанавливаются `issued_at` и QR‑код.
...
Дополнительно реализованы маршруты корзины (доступны только авторизованным пользователям):
...
**Статистика (админ‑раздел).** Администратор имеет доступ к странице статистики посещаемости/продаж:
- фильтры по постановке и диапазону дат;
- итоговые суммы (продано/проверено/выручка);
- агрегирование на основе таблицы `performance_stats`.
...
**Ограничение доступа (RBAC).** Реализовано разделение ролей:
- **Администратор (`users.is_admin = true`)** — управление сущностями (CRUD для постановок/актёров/сеансов), доступ к общей статистике и общим спискам.
- **Обычный пользователь** — покупка билетов, доступ только к **своей** корзине и **своим** заказам.

Технически разграничение реализовано через:
- middleware `auth` (доступ к персональным разделам только после входа);
- `app/Http/Middleware/AdminMiddleware.php` (запрет не‑админам, ответ HTTP 403);
- проверки владельца данных в `OrderController` и `TicketController` (пользователь не может просматривать/изменять чужие заказы и билеты).
...
**Примечание для Windows/учебной среды:** в проекте дополнительно настроено хранение на диске `public` так, чтобы загруженные файлы сохранялись в каталог `public/storage` и были доступны по `/storage/...` без обязательного использования симлинков.
...
**Результаты:**
- Все тесты проходят успешно (53 теста, 133 утверждения).
